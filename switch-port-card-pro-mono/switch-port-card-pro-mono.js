...
class SwitchPortCardProMono extends HTMLElement {
  constructor() {
    super();
  ...
  static getConfigElement() {
    return document.createElement("switch-port-card-pro-mono-editor");
  }
  ...
  setConfig(config) {
    ...
    if (config.exclude_ports !== undefined) {
      if (!Array.isArray(config.exclude_ports) ||
          config.exclude_ports.some(p => typeof p !== 'number' || !Number.isInteger(p) || p < 1)) {
        throw new Error("switch-port-card-pro-mono: exclude_ports must be an array of positive integers, e.g. [1, 2]");
      }
    }

    if (config.port_labels !== undefined) {
      if (typeof config.port_labels !== 'object' || Array.isArray(config.port_labels)) {
        throw new Error("switch-port-card-pro-mono: port_labels must be a map of port numbers to label strings, e.g. {6: LAN, 7: WAN}");
      }
    }
  ...
  set hass(hass) {
    ...
    if (!this._config) {
      console.warn("switch-port-card-pro-mono: config not set yet");
      return;
    }
  ...
  _applyPortsPerRowToGrid(gridElement) {
    ...
    gridElement.style.setProperty('--ports-per-row', portsPerRow);
  }
  _createSkeleton() {
    ...
    this.shadowRoot.innerHTML = `
        ...
        * { box-sizing: border-box; }
        ...
        .gauge-fill {
          ...
          background: linear-gradient(90deg,
            var(--c-ok, #4cad7f),
            var(--c-warn, #e8a444) 50%,
            #f85149
          );
        }

        /* ── Port-type section labels ── */
        .section-label{
          font-size:10px;
          font-weight:400;
          color: var(--c-muted);
        ...
        .port-row {
          ...
          justify-content: center;
          font-variant-numeric: tabular-nums;
          font-family: var(--font-mono, ui-monospace, 'Roboto Mono', monospace);
        ...
        .port.off{ background: var(--c-surface); border: 1px solid var(--c-border); opacity:0.9; color: var(--c-muted); }
      ...
      <div class="card">
        <div class="header">
          <span class="header-title" id="title">Switch</span>
          <div class="header-right">
            <span id="bandwidth" style="font-size:12px;color:var(--c-muted);font-family:var(--font-mono,monospace);font-variant-numeric:tabular-nums;min-width:7ch;text-align:right"></span>
        ...
        <div class="ports-section ${c}" style="padding:0 14px">
  ...
  _applyPortsPerRow() {
    ...
    if (copperGrid && sfpGrid) {
      ...
      copperGrid.style.setProperty('--ports-per-row', portsPerRow);
      sfpGrid.style.gridTemplateColumns = gridStyle;
      sfpGrid.style.setProperty('--ports-per-row', portsPerRow);
    }
  }
  ...
  _render() {
    ...
    if (Object.keys(e).length===0) {
      ...
      if (titleEl) titleEl.textContent = "No data — check integration";
      return;
    }
    ...
    if (this._config.show_total_bandwidth !== false && bw?.state) {
      ...
      let val = Number(bw.state);
      const u = (bw.attributes?.unit_of_measurement || "").toLowerCase();
      let maxInMbps = (this._config.max_bandwidth_gbps || 100) * 1000;

      if (u.includes("gbit") || (u.includes("mbit") && val > 1000)) {
        val = val / 1000;
        maxInMbps = this._config.max_bandwidth_gbps || 100;
      } else if (u.includes("bit/s") && !u.includes("mbit") && !u.includes("kbit") && !u.includes("gbit")) {
        val = val / 1e6;
      } else if (u.includes("kbit")) {
        val = val / 1e3;
    ...
    const excludePorts = new Set(this._config.exclude_ports || []);
    const visiblePorts = [];
    for (const port of allPorts) {
      const { i, ent } = port;

      if (excludePorts.has(i)) continue; // excluded by config
    ...
    const renderPortRow = (portList, container) => {
      portList.forEach(({ i, ent, isOn, traffic }) => {
        ...
        const customLabel = (this._config.port_labels || {})[i] || (this._config.port_labels || {})[String(i)] || null;
        const portDisplay = customLabel || String(i);
        const portNumFontSize = portDisplay.length > 4 ? '0.65em' : portDisplay.length > 2 ? '0.80em' : null;
        const vlan = ent.attributes?.vlan_id;
        ...
        div.innerHTML = `
          <div class="row1">
            ...
            <span class="port-num"${portNumFontSize ? ` style="font-size:${portNumFontSize}"` : ''}>${portDisplay}</span>
...
class SwitchPortCardProMonoEditor extends HTMLElement {
  setConfig(config) {
  ...
  // Prevent infinite re-rendering loop
    ...
    this.innerHTML = `
      ...
      <div style="padding:16px">
        ...
        <div class="field">
          ...
          <input type="text" data-key="exclude_ports" placeholder="e.g. 1, 2" value="${(this._config.exclude_ports || []).join(', ')}">
          <span class="hint">Comma-separated port numbers to hide</span>
        </div>
        <div class="field">
          <label>Port Labels</label>
          <input type="text" data-key="port_labels" placeholder="e.g. 6:LAN, 7:WAN" value="${Object.entries(this._config.port_labels || {}).map(([k,v]) => `${k}:${v}`).join(', ')}">
          <span class="hint">Replaces port number — e.g. 6:LAN, 7:WAN</span>
        </div>
      </div>
    `;
    ...
    this.querySelectorAll("[data-key]").forEach(el => {
      el.addEventListener("change", () => {
        ...
        let value = el.tagName === "HA-CHECKBOX" ? el.checked : el.value;

        if (key === "exclude_ports") {
        ...
        if (key === "port_labels") {
          value = {};
          el.value.split(",").forEach(pair => {
            const [k, v] = pair.split(":").map(s => s.trim());
            const port = parseInt(k, 10);
            if (!isNaN(port) && port > 0 && v) value[port] = v;
          });
        }
...
customElements.define("switch-port-card-pro-mono", SwitchPortCardProMono);
customElements.define("switch-port-card-pro-mono-editor", SwitchPortCardProMonoEditor);

(function () {
  window.customCards = window.customCards || [];
  window.customCards.push({
    type: "switch-port-card-pro-mono",
    name: "Switch Port Card Pro Mono",
